...
if not Path.exists(case_path):
    Path.mkdir(case_path) # 若不存在logs文件夹，则自动创建
...
class HandleLog():
    ...
    def __init__(self, log_name):
        ...
        # 其中，[日志名称]为调用日志时的传入参数
        self.__all_log_path = os.path.join(case_path, self.__now_time  + ".log")  # 收集所有日志信息文件
        self.__error_log_path = os.path.join(case_path,
                                             self.__now_time + "_error" + ".log")  # 收集错误日志信息文件
    ...
    def __console(self, level, message):
        ...
        if level == 'info':
            ...
            self.__logger.error(message)  # exc_info=True, stack_info=True 用于在日志中记录堆栈信息，方便查看日志进行调试
        elif level == 'critical':
            self.__logger.critical(message)  # exc_info=True, stack_info=True 用于在日志中记录堆栈信息，方便查看日志进行调试
...
logger= HandleLog(os.path.split(__file__)[-1].split(".")[0])
...
if __name__ == '__main__':
    ...
    log.warning("这是警告信息")
    log.error("这是错误日志信息")
    log.critical("这是严重级别信息")#!/usr/bin/env python
# encoding: utf-8
